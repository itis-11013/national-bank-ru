...
public class CentralServiceImpl<T, E> implements CentralService<T, E> {
    ...
    public E updateEntity(String requestPath, UUID uuid, T data) throws CentralResponseException {
        CentralResponse<E> response = centralClient.getWebClient().patch()
    ...
    public E getEntity(String requestPath, UUID uuid) throws CentralResponseException {
        CentralResponse<E> response = centralClient.getWebClient().get()
                .uri(uriBuilder -> uriBuilder
                        .path(requestPath + "/{id}")
                        .build(uuid))
                .retrieve()
    ...
    public void deleteEntity(String requestPath, UUID uuid) throws CentralResponseException {
        CentralResponse<UUID> response = centralClient.getWebClient().delete()
                .uri(uriBuilder -> uriBuilder
                        .path(requestPath + "/{id}")
                        .build(uuid)).retrieve()
                .bodyToMono(new ParameterizedTypeReference<CentralResponse<UUID>>() {
    ...
    public E getResponseBody(CentralResponse<E> response) {
        return (response == null || response.isFailure()) ? null : response.getData();
    }
}
